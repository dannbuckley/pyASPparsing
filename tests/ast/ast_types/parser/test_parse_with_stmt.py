        ...
        [
            ...
            (
                ...
                [
                    AssignStmt(
                        LeftExpr(
                            QualifiedID([Token.identifier(14, 19, dot_start=True)])
                        ),
                        ConstExpr(Token.string_literal(22, 38))
                    )
                ]
            )
        ]
)
def test_parse_with_stmt(codeblock: str, exp_with_expr: Expr, exp_block_stmt_list: typing.List[BlockStmt]):
